...
@section('breadcrumb', 'Admin / Universities / Programs / Edit')
    ...
    <div class="max-w-5xl">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl overflow-hidden">
            <div class="p-5 border-b border-slate-200">
                <div class="text-xl font-semibold">Edit Program</div>
                <div class="text-sm text-slate-500">{{ $university->name }}</div>
            </div>

            <form class="p-5 space-y-5" method="POST"
                action="{{ route('universities.programs.update', [$university, $program]) }}">
                @csrf
                @method('PUT')
